...
class ParticleSwarm
  ...
  def initialize(input)
    ...
    remove_collided_particles
  end

  def closest_to_origin
    particles.min_by { |particle| particle.distance_from_position([0, 0, 0]) }
  end
  ...
  def remove_collided_particles
    particles.group_by(&:position)
      .select { |p, particles| particles.length > 1 }
      .flat_map(&:last)
      .each do |particle|
  ...
  def closest_to_origin_at_time(n)
    particles.min { |particle| particle.distance_from_position_at_time([0, 0, 0], n) }
  end

  class Particle
    attr_reader :id, :x, :y, :z, :vx, :vy, :vz, :ax, :ay, :az

    def position
      [x,y,z]
    end

    def initialize(id, x, y, z, vx, vy, vz, ax, ay, az)
      @id = id
      @x = x
      @y = y
      @z = z
    ...
    def self.parse(id, input)
      matches = input.match(/p=<(.*)>, v=<(.*)>, a=<(.*)>/)
      Particle.new(id,
                   *matches[1].split(",").map(&:strip).map(&:to_i),
                   *matches[2].split(",").map(&:strip).map(&:to_i),
                   *matches[3].split(",").map(&:strip).map(&:to_i))
    end
    ...
    def distance_from_position((x, y, z))
      (self.x - x).abs + (self.y - y).abs + (self.z - z).abs
    end

    def distance_from_position_at_time((x, y, z), t)
      xt, yt, zt = position_at_time(t)
      ((xt - x) + (yt - y) + (zt - z)).abs
    end
  ...
  def parse(input)
    input.map.with_index { |line, i| Particle.parse(i, line) }
  end
end

describe ParticleSwarm do
  specify "the example" do
    ...
    swarm = ParticleSwarm.new(input)

    expect(swarm.closest_to_origin.id).to eq 0
    swarm.tick
    require 'pry'; binding.pry
    expect(swarm.closest_to_origin.id).to eq 1
    swarm.tick
    expect(swarm.closest_to_origin.id).to eq 1
    swarm.tick
    expect(swarm.closest_to_origin.id).to eq 0
...
1_000_000.times do |i|
  if i % 1000 == 0
    puts "Closest particle: #{swarm.closest_to_origin.id}"
    puts "Particles left: #{swarm.particles.length}"
  end
  swarm.tick
end
puts swarm.closest_to_origin.id
