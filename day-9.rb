...
def parse_stream(input)
  StreamNode.new.tap do |stream|
    while input.first
      initial_length = input.length
      stream.children << parse_node(input)
      break if initial_length == input.length
    end
  end
end
...
def parse_node(input)
  case(input.first)
  when "{" then parse_group(input)
  when "<" then parse_garbage(input)
  else
    raise "unexpected node input `#{input.first}`"
  end
end

def parse_group(input)
  GroupNode.new.tap do |group|
    input.shift
    # parse first child
    if input.first == "}"
      input.shift
    else
      next_char = input.first
      loop do
        group.children << parse_node(input)
        break unless (next_char = input.shift) == ","
      end
      raise "WHAT" unless next_char == "}"
    end
  end
end

def parse_garbage(input)
  GarbageNode.new.tap do |garbage|
    input.shift
    while (next_char = input.shift) != ">"
      if next_char == "!"
        input.shift
      else
        garbage.text << next_char
      end
    end
  end
end
...
class GroupNode
  ...
  def score(base = 1)
    base + children.sum { |child| child.score(base + 1) }
  end

  def text_length
    children.sum(&:text_length)
  end
end
...
class GarbageNode
  ...
  def score(*args); 0; end
  def count(*args); 0; end
...
describe "parse_group" do
  GROUP_TESTS = [
    ...
    ["{{<!!>},{<!!>},{<!!>},{<!!>}}", 5, 9],
    ["{{<a!>},{<a!>},{<a!>},{<ab>}}", 2, 3]
  ].each do |input, count, score|
    group = parse_group(input.chars)

    specify "#{input} has a count of #{count}" do
      expect(group.count).to eq count
    end

    specify "#{input} has a score of #{score}" do
      expect(group.count).to eq count
    end
  end
end

describe "parse_garbage" do
  [
    ['<>', 0],
    ...
    ['<!!!>>', 0],
    ['<{o"i!a,<{i<a>', 10],
  ].each do |input, text_length|
    garbage = parse_garbage(input.chars)

    specify "#{input} has a text length of #{text_length}" do
      expect(garbage.text_length).to eq text_length
    end
  end
end
