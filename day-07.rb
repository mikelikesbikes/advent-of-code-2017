...
class Tower
  ...
  def inspect
    %Q(#<Tower name="#{name}" weight=#{weight} total_weight=#{total_weight} children="#{children.map(&:name).join(", ")}" parent="#{parent&.name}">)
  end
  ...
  def unbalanced_child
    children.group_by(&:total_weight).values.find(nil) { |t| t.length == 1 }&.first
  end
end

def construct_towers(towers)
  tower_index = Hash.new { |h, k| h[k] = Tower.new(k) }
  towers.each_with_object(tower_index) do |tower, index|
    _, name, weight, _, children = tower.match(TOWER_REGEX).to_a
    tower = index[name]
    tower.weight = weight.to_i
    if children
      children.split(",").each do |child|
        child_tower = index[child.strip]
        tower.children << child_tower
        child_tower.parent = tower
      end
    end
  end
end

tower_index = construct_towers(towers)
base_tower = tower_index.values.find { |tower| tower.parent.nil? }

def balance_tower(tower, target_weight = nil)
  return unless tower.children

  # if the children are balanced, then the tower's weight should be adjusted
  unless unbalanced_child = tower.unbalanced_child
    tower.weight = target_weight - tower.children.sum(&:total_weight)
    return tower
  end

  balance_tower(unbalanced_child, (tower.children.map(&:total_weight) - [unbalanced_child.total_weight]).first)
end

p balance_tower(base_tower)
